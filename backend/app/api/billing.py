...
async def create_checkout(request: Request, body: CheckoutRequest, user: dict = Depends(get_current_user)):
...
@limiter.limit("5/minute")
async def create_portal(request: Request, user: dict = Depends(get_current_user)):
